	...
	static class ManulToolbar
	{
		...
		static void PerformButtonAction(ManulToolbarEntry entry)
		{
			...
			else
			{
				...
				{
					System.Action action = CreateAction(a);
		...
		static System.Action CreateAction(ManulToolbarAction toolbarAction)
		{
			...
			switch (toolbarAction.buttonType)
			{
				...
				case ToolbarButtonType.StaticMethod:
					...
					return () =>
					{
						System.AppDomain.CurrentDomain.GetClass(toolbarAction.className).GetMethod(toolbarAction.methodName, bindingFlags).Invoke(null, null);
					};
				...
				case ToolbarButtonType.ComponentMethod:
					...
					return () =>
					{
						...
						if (GO == null)
						{
							ManulToolbarMessages.ShowMessage(Message.CantFindGameObject, MessageType.Error, new string[1] { toolbarAction.objectName });
							return;
						}

						UnityEngine.Object O = GO.GetComponent(System.Type.GetType(toolbarAction.className));

						O.GetType().GetMethod(toolbarAction.methodName, bindingFlags).Invoke(O, null);
					};
				...
				case ToolbarButtonType.ObjectOfTypeMethod:
					...
					return () =>
					{
						Type objectType = GetClass(System.AppDomain.CurrentDomain, toolbarAction.className);

						UnityEngine.Object O = UnityEngine.Object.FindObjectOfType(System.Type.GetType(toolbarAction.className));
				...
				case ToolbarButtonType.OpenFolder:
					...
					return () =>
					{
						...
						if (obj == null)
						{
							ManulToolbarMessages.ShowMessage(Message.CantFindFolder, MessageType.Error, new string[1] { toolbarAction.className });
							return;
						}
				...
				case ToolbarButtonType.FindGameObjectInScene:
					...
					return () =>
					{
						...
						if (GO == null)
						{
							ManulToolbarMessages.ShowMessage(Message.CantFindGameObject, MessageType.Error, new string[1] { toolbarAction.className });
							return;
						}

						Selection.activeObject = GO;
					};
				case ToolbarButtonType.PlayGame:
					return () =>
					    {
					        // Nếu game đang chạy, dừng lại trước
					        if (EditorApplication.isPlaying)
					        {
					            EditorApplication.isPlaying = false;

					            // Đảm bảo game được dừng trước khi chạy lại
					        ...
					        {
					            // Nếu game chưa chạy, thì chạy game
					            EditorApplication.isPlaying = true;
					        }
					    };
				default:
					...
					if (entry.type == ToolbarEntryType.Button && entry.labelText == "Toolbar" && entry.actions != null)
					{
						if (entry.actions.Count == 2)
						{
							if (entry.actions[0].buttonType == ToolbarButtonType.StaticMethod &&
								entry.actions[0].className == "ManulToolbar" &&
